...
fn get_codes(path: &str, io: usize) -> Vec<Vec<String>> {
    ...
    for line in lines {
        ...
        let signals: Vec<String> = line.split(" ").map(|s|string_sort(&s.to_string())).collect();
        ret.push(signals);
    }
...
fn decode_input(input: &Vec<String>) -> HashMap<u32, String> {
    ...
    for code in input {
        match code.len() {
            2 => {ret.insert(1, code.to_string());},
            4 => {ret.insert(4, code.to_string());},
            3 => {ret.insert(7, code.to_string());},
            7 => {ret.insert(8, code.to_string());},
            _ => {},
        };
    }
...
fn main() {
    ...
    inputs.iter().for_each(|i|{
        ...
        let decoded_output = outputs[counter].iter()
            .map(|i| mh[i])
            .rev()
            .enumerate()
