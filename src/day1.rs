fn main()
{
    ...
    let result_part1 = input.windows(2).filter(|i| { i[1] > i[0] }).count();
    println!("Part 1: the result is {}", result_part1);
    ...
    let result_part2 = input.windows(3).fold(Vec::<u32>::new(), |mut a, i| {
        a.push(i.iter().sum());
        a
    }).windows(2).filter(|i| { i[1] > i[0] }).count();

    println!("Part 2: the result is {}", result_part2);
}
